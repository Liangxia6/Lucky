...
Server::Server(EventLoop *loop,
    ...
    , started_(0)
{
    // 当有新用户连接时，Acceptor类中绑定的acceptChannel_会有读事件发生，执行handleRead()调用Server::newConnection回调
...
void Server::newConnection(int sockfd, const Address &peerAddr)
{
    ...
    // 下面的回调都是用户设置给Server => Connection的，至于Channel绑定的则是Connection设置的四个，handleRead,handleWrite... 这下面的回调用于handlexxx函数中
