    ...
    , isCallPendFunc_(false)
{
    ...
    if (t_LoopinThread)
    {
        log<FATAL>("Another EventLoop %p exists in this thread %d\n", t_LoopinThread, threadId_);
    } 
    else 
    {
        t_LoopinThread = this;
    ...
    wakeupChannel_->setReadCallback(std::bind(&EventLoop::handelwakeupRead, this));
...
void EventLoop::loop()
{
    ...
    log<INFOM>("EventLoop %p start looping\n", this);

    while (!quit_)
    {
        //初始化activeChannels列表
        activeChannels_.clear();
        //开启epoll_wait,填充activeChannels列表
    ...
    log<INFOM>("EventLoop %p stop looping.\n", this);
    looping_ = false;
}
...
void EventLoop::wakeup() 
{
    ...
    if (n != sizeof(u64one))
    {
        log<ERROR>("EventLoop::wakeup() writes %lu bytes instead of 8\n", n);
    }
}
...
void EventLoop::handelwakeupRead()
{
    ...
    if (n != sizeof(u64one))
    {
        log<ERROR>("EventLoop::HandleRead() writes %lu bytes instead of 8\n", n);
    }
}
